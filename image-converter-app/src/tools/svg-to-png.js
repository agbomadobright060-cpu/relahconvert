...
const bg = '#F2F2F2'
const toolName  = (t.nav_short && t.nav_short['svg-to-png']) || 'SVG to PNG'
const seoData   = t.seo && t.seo['svg-to-png']
const descText  = seoData ? seoData.h2a : 'Convert SVG to PNG free. Set custom width. No upload required.'
...
document.querySelector('#app').innerHTML = `
  <div style="max-width:900px;margin:32px auto;padding:0 16px 60px;font-family:'DM Sans',sans-serif;">
    ...
    <div id="actionRow">
      <button class="action-btn" id="convertBtn" disabled style="background:var(--btn-disabled);opacity:0.7;cursor:not-allowed;">Convert to PNG</button>
...
function addFiles(newFiles) {
  Array.from(newFiles).forEach(file => {
    ...
    dlLink.className = 'dl-link'; dlLink.textContent = '\u2B07 Download PNG'
...
convertBtn.addEventListener('click', async () => {
  ...
  for (const f of files) {
    const blob = await svgToImage(f.file, width, 'png')
    const rawName = f.file.name.replace(/\.[^.]+$/, '') + '.png'
  ...
  statusText.textContent = files.length > 1 ? `${files.length} PNGs ready.` : 'PNG ready.'
...
;(function injectSEO() {
  const seo = t.seo && t.seo['svg-to-png']
  if (!seo) return
