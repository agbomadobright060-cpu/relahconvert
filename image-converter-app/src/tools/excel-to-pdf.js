// Excel → PDF tool with bulk processing (up to 10 files per session).
// Same Path-B architecture as Word→PDF: browser uploads directly to
// CloudConvert via signed URLs brokered by our Pages Function. CC_API_KEY
// is never exposed. CloudConvert is not named anywhere on the tool page;
// disclosure lives in the privacy policy.
...
const toolName     = (t.nav_short && t.nav_short[SLUG]) || 'Excel to PDF'
const seoData      = t.seo && t.seo[SLUG]
const descText     = t.excelpdf_desc || 'Convert one or multiple Excel files to PDF.'
const selectLbl    = t.excelpdf_select   || 'Select Excel files'
const dropHint     = t.excelpdf_drop_hint || 'or drop .xlsx / .xls files anywhere'
const convertLbl   = t.excelpdf_convert_btn || 'Convert to PDF'
const downloadAllLbl = t.excelpdf_download_all || 'Download all as ZIP'
const retryLbl     = t.excelpdf_retry || 'Retry'
const removeLbl    = t.remove || 'Remove'
const limitBannerLbl = t.excelpdf_limit_banner || 'Free limit: 10 files per session. Upgrade for more.'
const upgradeBtnLbl  = t.excelpdf_upgrade || 'Upgrade'

const STATUS_LABELS = {
  pending:    t.excelpdf_status_pending    || 'Ready',
  uploading:  t.excelpdf_status_uploading  || 'Uploading…',
  converting: t.excelpdf_status_converting || 'Converting…',
  done:       t.excelpdf_status_done       || 'Done',
  error:      t.excelpdf_status_error      || 'Failed',
}
const PROGRESS_LABEL_FMT = t.excelpdf_progress_fmt || 'Converting {current} of {total}…'
const SUMMARY_LABEL_FMT  = t.excelpdf_summary_fmt  || '{ok} of {total} converted'
const ERR = {
  too_large:           t.excelpdf_err_too_large           || 'File too large (max 25 MB per file).',
  total_too_large:     t.excelpdf_err_total_too_large     || 'Total batch size too large (max 100 MB).',
  too_many:            t.excelpdf_err_too_many            || 'Maximum 10 files per session on the free tier.',
  invalid_format:      t.excelpdf_err_invalid_format      || 'Please select Excel files (.xlsx or .xls).',
  rate_limited:        t.excelpdf_err_rate_limited        || 'Server busy. Try again in a moment.',
  quota_exhausted:     t.excelpdf_err_quota_exhausted     || 'Free conversion limit reached. Try again tomorrow.',
  service_unreachable: t.excelpdf_err_service_unreachable || 'Conversion service is unreachable. Try again shortly.',
  service_error:       t.excelpdf_err_service_error       || 'Conversion service returned an error.',
  password_protected:  t.excelpdf_err_password_protected  || 'File is password-protected. Remove the password and retry.',
  file_corrupted:      t.excelpdf_err_file_corrupted      || 'File appears corrupted.',
  unsupported_content: t.excelpdf_err_unsupported_content || 'File contains unsupported content.',
  conversion_failed:   t.excelpdf_err_conversion_failed   || 'Conversion failed.',
  upload_failed:       t.excelpdf_err_upload_failed       || 'Upload failed.',
  timeout:             t.excelpdf_err_timeout             || 'Conversion took too long.',
  generic:             t.excelpdf_err_generic             || 'Something went wrong.',
}
...
document.querySelector('#app').innerHTML = `
  <div style="max-width:700px;margin:32px auto;padding:0 16px 60px;font-family:'DM Sans',sans-serif;">
    ...
    <input type="file" id="fileInput" accept=".xlsx,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel" multiple style="display:none;" />
...
function addFiles(files) {
  ...
  for (const f of files) {
    ...
    if (!(name.endsWith('.xlsx') || name.endsWith('.xls'))) { invalidCount++; continue }
  ...
  // Surface a single, prioritized error message
  if (limitHit) {
...
function updateConvertButton() {
  ...
  // Show "Download all as ZIP" if at least 2 succeeded
...
async function processOne(entry) {
  ...
  if (result.ok) {
    // Fetch the PDF blob now so the "Download all as ZIP" button can bundle it locally
    try {
...
function downloadEntry(entry) {
  ...
  const baseName = (entry.name || 'spreadsheet').replace(/\.[^.]+$/, '')
...
async function downloadAllAsZip() {
  ...
  const zip = new JSZip()
  for (const e of succ) {
    const baseName = (e.name || 'spreadsheet').replace(/\.[^.]+$/, '')
...
function buildNextSteps() {
  ...
  const links = [
    { slug: 'word-to-pdf',   label: (t.nav_short && t.nav_short['word-to-pdf'])   || 'Word to PDF' },
