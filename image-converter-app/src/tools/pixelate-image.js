...
document.querySelector('#app').innerHTML = `
  <div id="mainContainer" style="max-width:700px; margin:32px auto; padding:0 16px 60px; font-family:'DM Sans',sans-serif; transition:max-width 0.3s;">
    ...
    <div id="uploadArea" style="margin-bottom:16px;">
      ...
      <label for="fileInput" id="dropZone" style="display:flex;flex-direction:column;align-items:center;justify-content:center;margin-top:16px;padding:48px 24px;border:2px dashed var(--border-light);border-radius:14px;cursor:pointer;transition:border-color 0.2s,background 0.2s;background:var(--bg-card);" onmouseover="this.style.borderColor='var(--accent)';this.style.background='var(--accent-bg)'" onmouseout="this.style.borderColor='var(--border-light)';this.style.background='var(--bg-card)'">
...
downloadBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (resultBlobs.length === 1) {
      ...
      window.rcShowSaveButton?.(downloadBtn.parentElement, resultBlobs[0].blob, resultBlobs[0].name, 'pixelate-image')
    } else {
      const zip = new JSZip()
      ...
      a.click();if(window.showReviewPrompt)window.showReviewPrompt()
      window.rcShowSaveButton?.(downloadBtn.parentElement, zipBlob, 'pixelated-images.zip', 'pixelate-image')
    }
    buildNextSteps()
  } catch (e) {
