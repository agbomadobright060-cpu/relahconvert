...
injectHreflang('svg-to-jpg')

const t = getT()

const bg = '#F2F2F2'
const toolName  = (t.nav_short && t.nav_short['svg-to-jpg']) || 'SVG to JPG'
const seoData   = t.seo && t.seo['svg-to-jpg']
const descText  = seoData ? seoData.h2a : 'Convert SVG to JPG free. Set custom width. White background for transparency. No upload required.'
...
document.title = seoData ? seoData.page_title : 'SVG to JPG Converter Free | No Upload \u2014 RelahConvert'
...
document.querySelector('#app').innerHTML = `
  <div style="max-width:900px;margin:32px auto;padding:0 16px 60px;font-family:'DM Sans',sans-serif;">
    ...
    <div class="notice">\u2139\uFE0F ${t.svg_to_jpg_notice || 'Upload SVG files and convert them to JPG images. Transparent areas are filled with white. Set a custom width \u2014 height scales proportionally.'}</div>
    ...
    <div id="actionRow">
      <button class="action-btn" id="convertBtn">Convert to JPG</button>
...
const outWidthInp = document.getElementById('outWidth')
...
function addFiles(newFiles) {
  Array.from(newFiles).forEach(file => {
    ...
    dlLink.className = 'dl-link'; dlLink.textContent = '\u2B07 Download JPG'
...
function svgToImage(svgFile, width, format) {
...
convertBtn.addEventListener('click', async () => {
  ...
  const width = parseInt(outWidthInp.value) || 1024
  const results = []
  const usedNames = new Set()
  for (const f of files) {
    const blob = await svgToImage(f.file, width, 'jpg')
    const rawName = f.file.name.replace(/\.[^.]+$/, '') + '.jpg'
...
zipBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    a.href = URL.createObjectURL(zipBlob); a.download = 'jpg-files.zip'; a.click();if(window.showReviewPrompt)window.showReviewPrompt(); setTimeout(() => URL.revokeObjectURL(a.href), 10000)
  } catch(e) { alert('ZIP failed: ' + e.message) }
  zipBtn.textContent = dlZipBtn; zipBtn.disabled = false
})

;(function injectSEO() {
  const seo = t.seo && t.seo['svg-to-jpg']
  if (!seo) return
