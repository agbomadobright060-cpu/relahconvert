...
const seoResize = {
  ...
  it: {
    ...
    body: `<p>RelahConvert ridimensiona le immagini interamente nel tuo browser — nessun caricamento, nessun server, nessun rischio per la privacy.</p>`,
    ...
    faqs: [{ q: 'Come ridimensiono un\'immagine senza perdere qualità?', a: 'Ridurre le dimensioni generalmente preserva bene la qualità. Usa il blocco proporzioni per evitare distorsioni.' },{ q: 'Posso ridimensionare in percentuale?', a: 'Sì — passa a "In percentuale" e inserisci la scala desiderata.' },{ q: 'Conservate le mie immagini?', a: 'Mai. Tutta l\'elaborazione avviene localmente nel tuo browser.' }],
  ...
  ja: {
    ...
    body: `<p>RelahConvertはブラウザ内で完全に画像をリサイズします — アップロード不要、サーバー不要、プライバシーリスクなし。</p>`,
    h3why: '画像をリサイズする理由', why: '大きすぎる画像はウェブサイトを遅くし、プラットフォームの要件を満たしません。リサイズにより画像の読み込みが速くなります。',
    faqs: [{ q: '品質を落とさずにリサイズするには？', a: '寸法を縮小すると通常品質は維持されます。歪みを防ぐにはアスペクト比ロックを使用してください。' },{ q: 'パーセントでリサイズできますか？', a: 'はい — 「パーセント」に切り替えて希望のスケールを入力してください。' },{ q: '画像を保存しますか？', a: 'いいえ。すべての処理はブラウザ内でローカルに行われます。' }],
  ...
  ru: {
    ...
    body: `<p>RelahConvert изменяет размер изображений полностью в вашем браузере — без загрузки, без сервера, без рисков для конфиденциальности.</p>`,
    ...
    faqs: [{ q: 'Как изменить размер без потери качества?', a: 'Уменьшение размеров обычно хорошо сохраняет качество. Используйте блокировку соотношения сторон для предотвращения искажений.' },{ q: 'Можно ли изменить размер в процентах?', a: 'Да — переключитесь на «В процентах» и введите нужный масштаб.' },{ q: 'Вы сохраняете мои изображения?', a: 'Никогда. Вся обработка происходит локально в вашем браузере.' }],
  ...
  ko: {
    ...
    body: `<p>RelahConvert는 브라우저에서 완전히 이미지 크기를 조정합니다 — 업로드 없음, 서버 없음, 개인정보 위험 없음.</p>`,
    h3why: '이미지 크기를 조정하는 이유', why: '너무 큰 이미지는 웹사이트를 느리게 하고 플랫폼 요구 사항을 충족하지 못합니다. 크기 조정으로 빠른 로딩을 보장합니다.',
    faqs: [{ q: '품질 손실 없이 크기를 조정하려면?', a: '크기를 줄이면 일반적으로 품질이 잘 유지됩니다. 왜곡을 방지하려면 비율 잠금을 사용하세요.' },{ q: '백분율로 크기를 조정할 수 있나요?', a: '네 — "백분율"로 전환하고 원하는 스케일을 입력하세요.' },{ q: '이미지를 저장하나요?', a: '절대 아닙니다. 모든 처리는 브라우저에서 로컬로 수행됩니다.' }],
  ...
  zh: {
    ...
    body: `<p>RelahConvert完全在浏览器中调整图片大小 — 无需上传、无需服务器、无隐私风险。</p>`,
    h3why: '为什么要调整图片大小？', why: '过大的图片会拖慢网站速度，不符合平台要求。调整大小确保图片快速加载。',
    faqs: [{ q: '如何在不损失质量的情况下调整大小？', a: '缩小尺寸通常能很好地保持质量。使用宽高比锁定来防止变形。' },{ q: '可以按百分比调整吗？', a: '可以 — 切换到"按百分比"并输入所需的比例。' },{ q: '你们会保存我的图片吗？', a: '绝不会。所有处理都在浏览器中本地完成。' }],
  ...
  'zh-TW': {
    ...
    body: `<p>RelahConvert完全在瀏覽器中調整圖片大小 — 無需上傳、無需伺服器、無隱私風險。</p>`,
    h3why: '為什麼要調整圖片大小？', why: '過大的圖片會拖慢網站速度，不符合平台要求。調整大小確保圖片快速載入。',
    faqs: [{ q: '如何在不損失品質的情況下調整大小？', a: '縮小尺寸通常能很好地保持品質。使用寬高比鎖定來防止變形。' },{ q: '可以按百分比調整嗎？', a: '可以 — 切換到「按百分比」並輸入所需的比例。' },{ q: '你們會保存我的圖片嗎？', a: '絕不會。所有處理都在瀏覽器中本地完成。' }],
  ...
  bg: {
    ...
    body: `<p>RelahConvert преоразмерява изображения изцяло във вашия браузър — без качване, без сървър, без риск за поверителността.</p>`,
    ...
    faqs: [{ q: 'Как да преоразмеря без загуба на качество?', a: 'Намаляването на размерите обикновено запазва качеството добре. Използвайте заключване на пропорциите за избягване на изкривявания.' },{ q: 'Мога ли да преоразмеря в проценти?', a: 'Да — превключете на «В проценти» и въведете желания мащаб.' },{ q: 'Съхранявате ли изображенията ми?', a: 'Никога. Цялата обработка се извършва локално в браузъра ви.' }],
  ...
  ca: {
    ...
    body: `<p>RelahConvert redimensiona les imatges completament al vostre navegador — sense pujada, sense servidor, sense risc per a la privacitat.</p>`,
    ...
    faqs: [{ q: 'Com redimensiono sense perdre qualitat?', a: 'Reduir les dimensions normalment preserva bé la qualitat. Utilitzeu el bloqueig de proporcions per evitar distorsions.' },{ q: 'Puc redimensionar per percentatge?', a: 'Sí — canvieu a "Per percentatge" i introduïu l\'escala desitjada.' },{ q: 'Deseu les meves imatges?', a: 'Mai. Tot el processament es fa localment al vostre navegador.' }],
    links: [{ href: '/compress', label: 'Comprimir' },{ href: '/png-to-jpg', label: 'PNG a JPG' },{ href: '/jpg-to-webp', label: 'JPG a WebP' },{ href: '/jpg-to-png', label: 'JPG a PNG' }],
  },
  nl: {
    ...
    body: `<p>RelahConvert wijzigt het formaat van afbeeldingen volledig in je browser — geen upload, geen server, geen privacyrisico.</p>`,
    ...
    faqs: [{ q: 'Hoe verklein ik zonder kwaliteitsverlies?', a: 'Het verkleinen van afmetingen behoudt over het algemeen de kwaliteit goed. Gebruik de beeldverhoudingsvergrendeling om vervorming te voorkomen.' },{ q: 'Kan ik op percentage verkleinen?', a: 'Ja — schakel naar "Op percentage" en voer de gewenste schaal in.' },{ q: 'Bewaren jullie mijn afbeeldingen?', a: 'Nooit. Alle verwerking gebeurt lokaal in je browser.' }],
  ...
  el: {
    ...
    body: `<p>Το RelahConvert αλλάζει το μέγεθος εικόνων εξ ολοκλήρου στο πρόγραμμα περιήγησής σας — χωρίς μεταφόρτωση, χωρίς διακομιστή, χωρίς κίνδυνο απορρήτου.</p>`,
    ...
    faqs: [{ q: 'Πώς αλλάζω μέγεθος χωρίς απώλεια ποιότητας;', a: 'Η μείωση διαστάσεων διατηρεί γενικά καλά την ποιότητα. Χρησιμοποιήστε το κλείδωμα αναλογίας για αποφυγή παραμόρφωσης.' },{ q: 'Μπορώ να αλλάξω μέγεθος κατά ποσοστό;', a: 'Ναι — μεταβείτε στο «Κατά ποσοστό» και εισάγετε την επιθυμητή κλίμακα.' },{ q: 'Αποθηκεύετε τις εικόνες μου;', a: 'Ποτέ. Όλη η επεξεργασία γίνεται τοπικά στο πρόγραμμα περιήγησής σας.' }],
  ...
  hi: {
    ...
    body: `<p>RelahConvert आपके ब्राउज़र में पूरी तरह से छवियों का आकार बदलता है — कोई अपलोड नहीं, कोई सर्वर नहीं, कोई गोपनीयता जोखिम नहीं।</p>`,
    ...
    faqs: [{ q: 'गुणवत्ता खोए बिना आकार कैसे बदलें?', a: 'आयाम कम करने से आमतौर पर गुणवत्ता अच्छी बनी रहती है। विकृति से बचने के लिए पक्षानुपात लॉक का उपयोग करें।' },{ q: 'क्या मैं प्रतिशत में आकार बदल सकता हूँ?', a: 'हाँ — "प्रतिशत में" पर स्विच करें और वांछित स्केल दर्ज करें।' },{ q: 'क्या आप मेरी छवियाँ सहेजते हैं?', a: 'कभी नहीं। सारी प्रोसेसिंग आपके ब्राउज़र में स्थानीय रूप से होती है।' }],
  ...
  id: {
    ...
    body: `<p>RelahConvert mengubah ukuran gambar sepenuhnya di browser Anda — tanpa unggah, tanpa server, tanpa risiko privasi.</p>`,
    ...
    faqs: [{ q: 'Bagaimana mengubah ukuran tanpa kehilangan kualitas?', a: 'Mengurangi dimensi biasanya mempertahankan kualitas dengan baik. Gunakan kunci rasio aspek untuk mencegah distorsi.' },{ q: 'Bisakah saya mengubah ukuran dalam persentase?', a: 'Ya — beralih ke "Dalam persentase" dan masukkan skala yang diinginkan.' },{ q: 'Apakah Anda menyimpan gambar saya?', a: 'Tidak pernah. Semua pemrosesan dilakukan secara lokal di browser Anda.' }],
  ...
  ms: {
    ...
    body: `<p>RelahConvert mengubah saiz imej sepenuhnya dalam pelayar anda — tanpa muat naik, tanpa pelayan, tanpa risiko privasi.</p>`,
    ...
    faqs: [{ q: 'Bagaimana mengubah saiz tanpa kehilangan kualiti?', a: 'Mengurangkan dimensi biasanya mengekalkan kualiti dengan baik. Gunakan kunci nisbah aspek untuk mengelakkan herotan.' },{ q: 'Bolehkah saya mengubah saiz dalam peratusan?', a: 'Ya — tukar ke "Dalam peratusan" dan masukkan skala yang dikehendaki.' },{ q: 'Adakah anda menyimpan imej saya?', a: 'Tidak pernah. Semua pemprosesan dilakukan secara tempatan dalam pelayar anda.' }],
  ...
  pl: {
    ...
    body: `<p>RelahConvert zmienia rozmiar obrazów w całości w przeglądarce — bez przesyłania, bez serwera, bez ryzyka dla prywatności.</p>`,
    ...
    faqs: [{ q: 'Jak zmienić rozmiar bez utraty jakości?', a: 'Zmniejszanie wymiarów zwykle dobrze zachowuje jakość. Użyj blokady proporcji, aby uniknąć zniekształceń.' },{ q: 'Czy mogę zmienić rozmiar procentowo?', a: 'Tak — przełącz na „Procentowo" i wpisz żądaną skalę.' },{ q: 'Czy przechowujecie moje obrazy?', a: 'Nigdy. Całe przetwarzanie odbywa się lokalnie w przeglądarce.' }],
  ...
  sv: {
    ...
    body: `<p>RelahConvert ändrar storlek på bilder helt i din webbläsare — ingen uppladdning, ingen server, ingen integritetsrisk.</p>`,
    ...
    faqs: [{ q: 'Hur ändrar jag storlek utan kvalitetsförlust?', a: 'Att minska dimensionerna bevarar generellt kvaliteten väl. Använd bildförhållandelåset för att undvika förvrängning.' },{ q: 'Kan jag ändra storlek i procent?', a: 'Ja — växla till "I procent" och ange önskad skala.' },{ q: 'Sparar ni mina bilder?', a: 'Aldrig. All bearbetning sker lokalt i din webbläsare.' }],
  ...
  th: {
    ...
    body: `<p>RelahConvert ปรับขนาดรูปภาพทั้งหมดในเบราว์เซอร์ของคุณ — ไม่ต้องอัปโหลด ไม่ต้องใช้เซิร์ฟเวอร์ ไม่มีความเสี่ยงด้านความเป็นส่วนตัว</p>`,
    ...
    faqs: [{ q: 'ปรับขนาดโดยไม่สูญเสียคุณภาพได้อย่างไร?', a: 'การลดขนาดมักรักษาคุณภาพได้ดี ใช้ล็อกอัตราส่วนเพื่อป้องกันการบิดเบือน' },{ q: 'สามารถปรับขนาดเป็นเปอร์เซ็นต์ได้ไหม?', a: 'ได้ — สลับไปที่ "เป็นเปอร์เซ็นต์" แล้วป้อนสเกลที่ต้องการ' },{ q: 'คุณเก็บรูปภาพของฉันไหม?', a: 'ไม่เลย การประมวลผลทั้งหมดเกิดขึ้นในเบราว์เซอร์ของคุณ' }],
  ...
  tr: {
    ...
    body: `<p>RelahConvert görselleri tamamen tarayıcınızda boyutlandırır — yükleme yok, sunucu yok, gizlilik riski yok.</p>`,
    ...
    faqs: [{ q: 'Kalite kaybı olmadan boyut nasıl değiştirilir?', a: 'Boyutları küçültmek genellikle kaliteyi iyi korur. Bozulmayı önlemek için en-boy oranı kilidini kullanın.' },{ q: 'Yüzde olarak boyutlandırabilir miyim?', a: 'Evet — "Yüzde olarak"a geçin ve istediğiniz ölçeği girin.' },{ q: 'Görsellerimi saklıyor musunuz?', a: 'Asla. Tüm işlemler tarayıcınızda yerel olarak gerçekleşir.' }],
  ...
  uk: {
    ...
    body: `<p>RelahConvert змінює розмір зображень повністю у вашому браузері — без завантаження, без сервера, без ризику для конфіденційності.</p>`,
    ...
    faqs: [{ q: 'Як змінити розмір без втрати якості?', a: 'Зменшення розмірів зазвичай добре зберігає якість. Використовуйте блокування співвідношення сторін для запобігання спотворенню.' },{ q: 'Чи можна змінити розмір у відсотках?', a: 'Так — переключіться на «У відсотках» і введіть бажаний масштаб.' },{ q: 'Ви зберігаєте мої зображення?', a: 'Ніколи. Вся обробка відбувається локально у вашому браузері.' }],
  ...
  vi: {
    ...
    body: `<p>RelahConvert thay đổi kích thước ảnh hoàn toàn trong trình duyệt — không tải lên, không máy chủ, không rủi ro quyền riêng tư.</p>`,
    ...
    faqs: [{ q: 'Làm sao thay đổi kích thước mà không mất chất lượng?', a: 'Giảm kích thước thường giữ chất lượng tốt. Sử dụng khóa tỷ lệ khung hình để tránh méo.' },{ q: 'Có thể thay đổi kích thước theo phần trăm không?', a: 'Có — chuyển sang "Theo phần trăm" và nhập tỷ lệ mong muốn.' },{ q: 'Các bạn có lưu ảnh của tôi không?', a: 'Không bao giờ. Mọi xử lý diễn ra cục bộ trong trình duyệt của bạn.' }],
