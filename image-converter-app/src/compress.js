...
const seoCompress = {
  ...
  it: {
    ...
    body: `<p>La maggior parte degli strumenti di compressione online carica le tue immagini su un server remoto. RelahConvert funziona diversamente: la compressione avviene interamente nel tuo browser. Le tue immagini non lasciano mai il tuo dispositivo.</p>`,
  ...
  ja: {
    ...
    body: `<p>ほとんどのオンライン圧縮ツールは画像をリモートサーバーにアップロードします。RelahConvertは異なります：圧縮は完全にブラウザ内で行われます。画像がデバイスから離れることはありません。</p>`,
    h3why: '画像を圧縮する理由', why: '大きなファイルはウェブサイトを遅くし、不要なストレージを消費します。圧縮は視覚的な品質を維持しながらファイルサイズを削減します。',
    faqs: [{ q: '品質を落とさずに圧縮するには？', a: 'ほとんどの画像で、75〜85%の品質で元と見分けがつかないファイルが生成されます。' },{ q: '複数の画像を一度に圧縮できますか？', a: 'はい — 複数の画像を選択してください。複数ファイルはZIPで配信されます。' },{ q: 'どの形式に対応していますか？', a: 'JPG、PNG、WebPの圧縮に対応しています。' },{ q: '画像を保存しますか？', a: 'いいえ。すべての処理はブラウザ内でローカルに行われます。' }],
  ...
  ru: {
    ...
    body: `<p>Большинство онлайн-инструментов сжатия загружают ваши изображения на удалённый сервер. RelahConvert работает иначе: сжатие происходит полностью в вашем браузере. Ваши изображения никогда не покидают ваше устройство.</p>`,
    ...
    faqs: [{ q: 'Как сжать изображение без потери качества?', a: 'Для большинства изображений качество 75–85% даёт файлы, неотличимые от оригинала.' },{ q: 'Можно сжать несколько изображений сразу?', a: 'Да — выберите несколько изображений. Множественные файлы доставляются в ZIP.' },{ q: 'Какие форматы поддерживаются?', a: 'RelahConvert поддерживает сжатие JPG, PNG и WebP.' },{ q: 'Вы сохраняете мои изображения?', a: 'Никогда. Вся обработка происходит локально в вашем браузере.' }],
  ...
  ko: {
    ...
    body: `<p>대부분의 온라인 압축 도구는 이미지를 원격 서버에 업로드합니다. RelahConvert는 다릅니다: 압축이 완전히 브라우저에서 이루어집니다. 이미지가 기기를 떠나지 않습니다.</p>`,
    h3why: '이미지를 압축하는 이유', why: '큰 파일은 웹사이트를 느리게 하고 불필요한 저장 공간을 차지합니다. 압축은 시각적 품질을 유지하면서 파일 크기를 줄입니다.',
    faqs: [{ q: '품질 손실 없이 압축하려면?', a: '대부분의 이미지에서 75–85% 품질은 원본과 동일하게 보이는 파일을 생성합니다.' },{ q: '여러 이미지를 한 번에 압축할 수 있나요?', a: '네 — 여러 이미지를 선택하세요. 복수 파일은 ZIP으로 제공됩니다.' },{ q: '어떤 형식을 지원하나요?', a: 'JPG, PNG, WebP 압축을 지원합니다.' },{ q: '이미지를 저장하나요?', a: '절대 아닙니다. 모든 처리는 브라우저에서 로컬로 수행됩니다.' }],
  ...
  zh: {
    ...
    body: `<p>大多数在线压缩工具会将图片上传到远程服务器。RelahConvert不同：压缩完全在浏览器中进行。图片永远不会离开你的设备。</p>`,
    h3why: '为什么要压缩图片？', why: '大文件会拖慢网站速度并占用不必要的存储空间。压缩可以在保持视觉质量的同时减小文件大小。',
    faqs: [{ q: '如何在不损失质量的情况下压缩？', a: '大多数图片在75–85%质量下生成的文件与原件看起来完全相同。' },{ q: '可以一次压缩多张图片吗？', a: '可以 — 选择多张图片。多个文件以ZIP格式提供。' },{ q: '支持哪些格式？', a: '支持JPG、PNG和WebP压缩。' },{ q: '你们会保存我的图片吗？', a: '绝不会。所有处理都在浏览器中本地完成。' }],
  ...
  'zh-TW': {
    ...
    body: `<p>大多數線上壓縮工具會將圖片上傳到遠端伺服器。RelahConvert不同：壓縮完全在瀏覽器中進行。圖片永遠不會離開你的裝置。</p>`,
    h3why: '為什麼要壓縮圖片？', why: '大檔案會拖慢網站速度並佔用不必要的儲存空間。壓縮可以在保持視覺品質的同時減小檔案大小。',
    faqs: [{ q: '如何在不損失品質的情況下壓縮？', a: '大多數圖片在75–85%品質下生成的檔案與原件看起來完全相同。' },{ q: '可以一次壓縮多張圖片嗎？', a: '可以 — 選擇多張圖片。多個檔案以ZIP格式提供。' },{ q: '支援哪些格式？', a: '支援JPG、PNG和WebP壓縮。' },{ q: '你們會儲存我的圖片嗎？', a: '絕不會。所有處理都在瀏覽器中本地完成。' }],
  ...
  bg: {
    ...
    body: `<p>Повечето онлайн инструменти за компресиране качват изображенията ви на отдалечен сървър. RelahConvert работи различно: компресирането се извършва изцяло в браузъра. Изображенията ви никога не напускат устройството.</p>`,
  ...
  ca: {
    ...
    body: `<p>La majoria d'eines de compressió en línia pugen les imatges a un servidor remot. RelahConvert funciona diferent: la compressió es fa completament al navegador. Les imatges no surten mai del dispositiu.</p>`,
    ...
    faqs: [{ q: 'Com comprimir sense perdre qualitat?', a: 'Per a la majoria d\'imatges, una qualitat del 75–85% produeix fitxers idèntics a l\'original.' },{ q: 'Puc comprimir diverses imatges alhora?', a: 'Sí — seleccioneu diverses imatges. Els fitxers múltiples es lliuren en ZIP.' },{ q: 'Quins formats es suporten?', a: 'Suporta compressió JPG, PNG i WebP.' },{ q: 'Deseu les meves imatges?', a: 'Mai. Tot el processament es fa localment al navegador.' }],
    links: [{ href: '/resize', label: 'Redimensionar' },{ href: '/jpg-to-png', label: 'JPG a PNG' },{ href: '/jpg-to-webp', label: 'JPG a WebP' },{ href: '/png-to-jpg', label: 'PNG a JPG' }],
  },
  nl: {
    ...
    body: `<p>De meeste online compressietools uploaden je afbeeldingen naar een externe server. RelahConvert werkt anders: compressie vindt volledig in je browser plaats. Je afbeeldingen verlaten nooit je apparaat.</p>`,
  ...
  el: {
    ...
    body: `<p>Τα περισσότερα εργαλεία συμπίεσης ανεβάζουν τις εικόνες σας σε απομακρυσμένο διακομιστή. Το RelahConvert λειτουργεί διαφορετικά: η συμπίεση γίνεται εξ ολοκλήρου στο πρόγραμμα περιήγησής σας.</p>`,
  ...
  hi: {
    ...
    body: `<p>अधिकांश ऑनलाइन संपीड़न उपकरण आपकी छवियों को दूरस्थ सर्वर पर अपलोड करते हैं। RelahConvert अलग है: संपीड़न पूरी तरह से ब्राउज़र में होता है। छवियाँ कभी डिवाइस नहीं छोड़तीं।</p>`,
  ...
  id: {
    ...
    body: `<p>Sebagian besar alat kompresi online mengunggah gambar Anda ke server jarak jauh. RelahConvert berbeda: kompresi terjadi sepenuhnya di browser. Gambar Anda tidak pernah meninggalkan perangkat.</p>`,
    ...
    faqs: [{ q: 'Bagaimana mengompres tanpa kehilangan kualitas?', a: 'Untuk sebagian besar gambar, kualitas 75–85% menghasilkan file yang terlihat identik dengan aslinya.' },{ q: 'Bisakah mengompres banyak gambar sekaligus?', a: 'Ya — pilih beberapa gambar. File multiple dikirim dalam ZIP.' },{ q: 'Format apa yang didukung?', a: 'Mendukung kompresi JPG, PNG, dan WebP.' },{ q: 'Apakah Anda menyimpan gambar saya?', a: 'Tidak pernah. Semua pemrosesan dilakukan secara lokal di browser.' }],
  ...
  ms: {
    ...
    body: `<p>Kebanyakan alat pemampatan dalam talian memuat naik imej anda ke pelayan jauh. RelahConvert berbeza: pemampatan berlaku sepenuhnya dalam pelayar. Imej anda tidak pernah meninggalkan peranti.</p>`,
    ...
    faqs: [{ q: 'Bagaimana memampatkan tanpa kehilangan kualiti?', a: 'Untuk kebanyakan imej, kualiti 75–85% menghasilkan fail yang kelihatan sama dengan asal.' },{ q: 'Bolehkah memampatkan banyak imej sekaligus?', a: 'Ya — pilih beberapa imej. Fail berganda dihantar dalam ZIP.' },{ q: 'Format apa yang disokong?', a: 'Menyokong pemampatan JPG, PNG dan WebP.' },{ q: 'Adakah anda menyimpan imej saya?', a: 'Tidak pernah. Semua pemprosesan dilakukan secara tempatan dalam pelayar.' }],
  ...
  pl: {
    ...
    body: `<p>Większość narzędzi kompresji online przesyła obrazy na zdalny serwer. RelahConvert działa inaczej: kompresja odbywa się całkowicie w przeglądarce. Obrazy nigdy nie opuszczają urządzenia.</p>`,
  ...
  sv: {
    ...
    body: `<p>De flesta onlinekomprimeringsverktyg laddar upp bilder till en fjärrserver. RelahConvert fungerar annorlunda: komprimering sker helt i din webbläsare. Dina bilder lämnar aldrig din enhet.</p>`,
  ...
  th: {
    ...
    body: `<p>เครื่องมือบีบอัดออนไลน์ส่วนใหญ่จะอัปโหลดรูปภาพไปยังเซิร์ฟเวอร์ RelahConvert ทำงานต่างออกไป: การบีบอัดเกิดขึ้นทั้งหมดในเบราว์เซอร์ รูปภาพไม่เคยออกจากอุปกรณ์</p>`,
    ...
    faqs: [{ q: 'บีบอัดโดยไม่สูญเสียคุณภาพได้อย่างไร?', a: 'สำหรับรูปภาพส่วนใหญ่ คุณภาพ 75–85% สร้างไฟล์ที่ดูเหมือนต้นฉบับ' },{ q: 'บีบอัดหลายรูปพร้อมกันได้ไหม?', a: 'ได้ — เลือกหลายรูปภาพ ไฟล์หลายไฟล์จะส่งเป็น ZIP' },{ q: 'รองรับรูปแบบอะไรบ้าง?', a: 'รองรับการบีบอัด JPG, PNG และ WebP' },{ q: 'คุณเก็บรูปภาพของฉันไหม?', a: 'ไม่เลย การประมวลผลทั้งหมดเกิดขึ้นในเบราว์เซอร์' }],
  ...
  tr: {
    ...
    body: `<p>Çoğu çevrimiçi sıkıştırma aracı görsellerinizi uzak bir sunucuya yükler. RelahConvert farklı çalışır: sıkıştırma tamamen tarayıcınızda gerçekleşir. Görselleriniz cihazınızdan asla ayrılmaz.</p>`,
  ...
  uk: {
    ...
    body: `<p>Більшість онлайн-інструментів стиснення завантажують зображення на віддалений сервер. RelahConvert працює інакше: стиснення відбувається повністю у браузері. Зображення ніколи не залишають ваш пристрій.</p>`,
  ...
  vi: {
    ...
    body: `<p>Hầu hết công cụ nén trực tuyến tải ảnh lên máy chủ từ xa. RelahConvert khác biệt: nén diễn ra hoàn toàn trong trình duyệt. Ảnh không bao giờ rời khỏi thiết bị.</p>`,
